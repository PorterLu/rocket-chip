 ...
 *    sswi: Option[SSWIParams]      = Some(SSWIParams(baseAddress = ttt))
 *  )
 */
case class CLINTParams(
  ...
  require(!(!isACLINT && sswi.isDefined), "The sswi should not be specified when isACLINT = false")
}
